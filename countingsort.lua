...
function get_description()
	return { "THE SEQUENCE IS -1 TERMINATED", "READ THE SEQUENCE FROM IN", "SORT THE SEQUENCE", "WRITE THE SEQUENCE TO OUT" }
end
...
function get_streams()
	...
	for i = 1,38 do
		repeat
			input[i] = math.random(0, 9)
			output[i] = input[i]
		until zeros>1 or (zeros==1 and counts[input[i]]>0)
		
		if counts[input[i]]==0 then
			zeros = zeros - 1
		end
